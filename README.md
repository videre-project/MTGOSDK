...
> [!NOTE]
> This project requires the [.NET 10 SDK](https://dotnet.microsoft.com/download/dotnet/10.0) to be installed with Visual Studio 2026 (v18.0) or newer. This can also be installed in Visual Studio using the [Visual Studio Installer](https://learn.microsoft.com/en-us/visualstudio/install/install-visual-studio?view=visualstudio).
...
1.  **Stage 1: MTGO Runtime** — A base Wine environment with .NET 4.8 and fonts.
2.  **Stage 2: MTGOSDK** — Builds on the runtime to add the .NET 10 SDK and developer tools.

### Quick Start

Pre-built development images are available on Docker Hub as `videreproject/mtgosdk`.

| Tag | Purpose | Best for... |
|-----|---------|-------------|
| `:headless` | Virtual desktop (VNC) | Continuous CI, background tasks, less obtrusive dev |
| `:wayland` | Native host display | High-performance interactive use (GNOME/Plasma) |
| `:x11` | Shared X11 socket | Interactive use (macOS XQuartz, older Linux) |

To start a development environment using your local repository:

```bash
# Start a Wayland container (requires local xhost + permissions)
docker run -it --name mtgosdk-dev \
  -e DISPLAY=$DISPLAY \
  -e WAYLAND_DISPLAY=$WAYLAND_DISPLAY \
  -v /tmp/.X11-unix:/tmp/.X11-unix \
  -v ${XDG_RUNTIME_DIR}/wayland-0:/tmp/runtime-dir/wayland-0 \
  -v $(pwd):/workspace \
  videreproject/mtgosdk:wayland
```
...
$ wine-run MTGOSDK.Tests
```

> [!TIP]
> **Headless is recommended** for general development as it doesn't clutter your host desktop. Use **Wayland/X11** when you need to interact with the MTGO UI directly (e.g., debugging startup issues or login flows).
