...
<div align="center">
  ...
  <a href="https://www.nuget.org/packages/MTGOSDK">![NuGet](https://img.shields.io/nuget/v/MTGOSDK?label=NuGet&labelColor=3f4551&color=blue&logo=nuget)</a>
...
MTGOSDK is a C# library for interfacing with the **Magic: The Gathering Online (MTGO)** client. It provides common APIs for reading from and interacting with the MTGO client in real-time, enabling developers to build tools and applications that can interact with any running instance of MTGO.
...
- **Collection**: Query the MTGO collection for cards, decks, and other items.
...
- **Play**: Query events and read the current state of a game, including replays and game history.
- **Settings**: Read, export, and import user preferences and settings.
- **Trade**: Query and read trade offers, trade history, and any current trade sessions.
- **Users**: Query user profiles, including friends and other players.
- and more...

This SDK is lightweight (with minimal dependencies outside of the .NET runtime) and is designed to be easy to use and integrate into existing applications. It is built on top of the .NET Core runtime (with no native dependencies), and is compatible with .NET 5.0 and later.
...
>
> We recommend updating to the [latest commit in the `main` branch](https://github.com/videre-project/MTGOSDK/commits/main) as often as possible. This helps catch and fix any [Hyrum's Law](https://www.hyrumslaw.com/) dependency problems on an incremental basis between different versions of MTGO and the SDK.
>
> While regular releases are tagged and published on GitHub and NuGet, we recommend [building the SDK from source](#building-this-project) to ensure you have the latest features and bug fixes, and so that newer versions of MTGO can be targeted and verified by the SDK at build-time. Follow the [local package feed](#local-package-feed) instructions to reference the development build in your project.
