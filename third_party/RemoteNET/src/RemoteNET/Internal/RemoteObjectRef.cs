...
namespace RemoteNET.Internal
{
  internal class RemoteObjectRef
  {
    ...
    public MemberDump GetFieldDump(string name, bool refresh = false)
    {
      ...
      if (refresh)
      {
        _remoteObjectInfo = _creatingCommunicator
          .DumpObject(_remoteObjectInfo.PinnedAddress, _remoteObjectInfo.Type);
      }
    ...
    {
      ThrowIfReleased();
      return _creatingCommunicator
        .InvokeMethod(
            _remoteObjectInfo.PinnedAddress,
            _remoteObjectInfo.Type,
            methodName,
