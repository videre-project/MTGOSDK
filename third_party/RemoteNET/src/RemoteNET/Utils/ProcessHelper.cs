...
namespace RemoteNET.Utils
{
  ...
  {
    /// <summary>
    /// Gets the single process in the system with a given name. 'Root' means
    /// that if a process tree of several proceses with the same name exist it
    /// returns the tree's root.
    /// </summary>
    public static Process GetSingleRoot(string procName)
    {
      // Need to add ".exe" extensions so if 2 processes exists such that 1 has
      // a substring proc name of the other we'd still be able to tell them apart.
      //
      // Example:
      ...
      var possibleRoots = candidateProcs.Where(proc =>
      {
        ...
        // If we can't find the parent just go with this process.
        if (parentProc == null)
          return true;
      ...
      else
      {
        ...
        throw new TooManyProcessesException(
          $"Too many processes contains '{procName}' in their name.\n" +
